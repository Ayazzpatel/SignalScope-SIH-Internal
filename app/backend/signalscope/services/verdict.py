...
def make_ensemble_verdict(ai_probabilities: Sequence[float], ai_threshold: float) -> Verdict:
    """Flag the image as likely AI-generated when ANY model's probability reaches `ai_threshold`.

    Deliberately cautious: one detector catching a generator the others miss is enough. The cost is
    more real photos flagged, so there is no "uncertain" band here — only the two outcomes.
    """
    if not ai_probabilities:
        raise ValueError("make_ensemble_verdict() needs at least one probability")
    top = max(ai_probabilities)
    ...
    return Verdict(band=band, prob_ai=top, headline=headline, summary=summary, thresholds=thresholds)
